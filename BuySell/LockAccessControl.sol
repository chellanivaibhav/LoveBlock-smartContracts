...
contract LockAccessControl {
    ...
    function setBaseAddr(address _newBaseAddr) external onlyCEO {
        require(_newBaseAddr != address(0));

        baseAddr = _newBaseAddr;
    }
    ...
    function setBuySellStorageAddr(address _newBuySellStorage) external onlyCEO {
        require(_newBuySellStorage != address(0));

        buysellstorage = _newBuySellStorage;
    }
