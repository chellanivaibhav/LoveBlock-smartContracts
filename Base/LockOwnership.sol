...
contract LockOwnership is LockBase , ERC721 {
    ...
    function _owns(address _claimant, uint256 _tokenId) view  external RWAccess returns (bool) {
    ...
    /// @param _tokenId lock id, only valid when > 0
    function _approvedFor(address _claimant, uint256 _tokenId) RWAccess view external returns (bool) {
        return lockIndexToApproved[_tokenId] == _claimant;
    }

    /// @dev Marks an address as being approved for transferFrom(), overwriting any previous
    ///  approval. Setting _approved to address(0) clears all transfer approval.
    function _approve(uint256 _tokenId, address _approved) external RWAccess {
    ...
    whenNotPaused
    {
        ...
        this._transfer(msg.sender, _to, _tokenId);
    }
